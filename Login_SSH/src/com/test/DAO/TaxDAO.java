...
Session session = null;
	...
	public List testIsNull(String hql_test)  //测试是否在岗位流转表里
	{
		session = baseDAO.getSession();
		
		Query query_testNull = session.createQuery(hql_test);
		List list_testNull = query_testNull.list();
		session.close();
		return list_testNull;
		
	}
	public List businessReception(String post_id,String user_num,String hql)  //业务员业务操作,选择处理哪项业务，如X001、X002
	{
	    session = baseDAO.getSession();
		session.beginTransaction();  //开启事务	
		Query query_business = session.createQuery(hql);
	...
	public List getCollective_post(String hql)  //获得集体岗的用户编号
	{
		session = baseDAO.getSession();
		Query query_user = session.createQuery(hql);
		List list = query_user.list();
		session.close();
		return list;
